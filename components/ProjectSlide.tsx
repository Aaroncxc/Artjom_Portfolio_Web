...
export function ProjectSlide({ project, isActive }: ProjectSlideProps) {
  ...
  const isPultProject = project.slug === 'pult-vacuum';
  const pultMainVideoRatio = pultAspectRatios['pult-main-video'];
  const isPortraitMainVideo = isPultProject
    ? (typeof pultMainVideoRatio === 'number' ? pultMainVideoRatio < 1 : !!project.videoPortrait)
    : !!project.videoPortrait;

  const updatePultMediaMeta = (key: string, width: number, height: number) => {
    if (!isPultProject || width <= 0 || height <= 0) return;
  ...
  useEffect(() => {
    if (
      !isPultProject &&
      (Object.keys(pultAspectRatios).length > 0 || Object.keys(pultMediaDimensions).length > 0)
    ) {
      setPultAspectRatios({});
      setPultMediaDimensions({});
    }
  }, [isPultProject, pultAspectRatios, pultMediaDimensions]);
      ...
      <div className="snap-section flex flex-col justify-center px-4 sm:px-6 py-6 sm:py-8 lg:py-16 min-h-[100dvh]">
        <div className="max-w-5xl w-full mx-auto">
          ...
          >
            ...
            {/* Video Player - portrait (9:16) or landscape (16:9) */}
            <div 
              className={`relative group/video rounded-2xl overflow-hidden bg-black/5 ${isPortraitMainVideo ? 'mx-auto max-w-[min(100%,320px)]' : 'w-full'}`}
              style={{
                aspectRatio: isPultProject && typeof pultMainVideoRatio === 'number'
                  ? `${pultMainVideoRatio}`
                  : (isPortraitMainVideo ? '9/16' : '16/9'),
              }}
            >
              <video
                ref={videoRef}
            ...
            {/* Gallery - default 16:9, for PULT dynamic by media content ratio */}
            {project.gallery && project.gallery.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-medium text-mk-text mb-4">Weitere Ansichten</h3>
                <div className="grid grid-cols-1 gap-4">
                  {project.gallery.map((media, index) => {
                    const ratioKey = `pult-gallery-${index}`;
                    ...
                    const isTransparentImageCard = isPultProject && isImage;
                    const pultCardStyle = isPultProject
                      ? {
                          ...(hasKnownRatio ? { aspectRatio: `${ratio}` } : {}),
                          ...(dimensions ? { maxWidth: `${dimensions.width}px` } : {}),
                          marginLeft: 'auto',
                          marginRight: 'auto',
                        }
                      : undefined;
                    ...
                    return (
                      <div 
                        key={index}
                        className={`relative rounded-xl overflow-hidden group ${isTransparentImageCard ? 'bg-transparent' : 'bg-black/5'} ${isPultProject ? '' : 'aspect-video'} ${isImage ? (isPultProject ? 'cursor-pointer' : 'cursor-pointer border border-[rgba(28,28,28,0.1)] hover:border-[rgba(28,28,28,0.25)] transition-colors') : ''}`}
                        style={pultCardStyle}
                      ...
                      >
                        {media.type === 'video' ? (
                          <video
                            ...
                            className={
                              isPultProject
                                ? (hasKnownRatio ? 'w-full h-full object-contain' : 'block w-full h-auto object-contain')
                        ...
                        {media.title && (
                          <div className={`absolute bottom-0 left-0 right-0 px-3 py-1.5 bg-gradient-to-t ${isPultProject ? 'from-[rgba(28,28,28,0.5)]' : 'from-black/70'} to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`}>
      ...
      <div className="snap-section flex flex-col justify-center px-4 sm:px-6 py-6 sm:py-8 lg:py-16 min-h-[100dvh]">
        <div className="max-w-5xl w-full mx-auto">
          ...
          >
            ...
            {/* Gallery - additional images (click to enlarge) */}
            {project.gallery && project.gallery.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-medium text-mk-text mb-4">Weitere Ansichten</h3>
            ...
            {/* Lightbox Modal - click to enlarge gallery images */}
            <AnimatePresence>
              {lightboxImage && (
                <motion.div
                  initial={{ opacity: 0 }}
